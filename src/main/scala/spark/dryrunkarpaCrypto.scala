...
object dryrunkarpaCrypto {
  ...
  def main(args: Array[String]): Unit = {
    ...
    // Create Hive Internal table
    filtered_df_ethereum.write.mode(SaveMode.Overwrite).saveAsTable("scalagroup.Ethereum_FilteredByPrice1")
    ...
    mean_price_df_ethereum.write.mode(SaveMode.Overwrite).saveAsTable("scalagroup.ethereum_mean_price1")
    ...
    val mean_price_df_bitcoin = dropped_bitcoin_null_column.withColumn("bitcoin_mean_price1", lit(bitcoin_mean_price))
    ...
    df_join.show(false)
    df_join.write.mode(SaveMode.Overwrite).saveAsTable("scalagroup.bitcoinjoinethereum1")
